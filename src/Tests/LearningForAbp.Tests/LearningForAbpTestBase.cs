...
{
    public abstract class LearningForAbpTestBase : AbpIntegratedTestBase<LearningForAbpTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected LearningForAbpTestBase()
        {
        ...
        protected void UsingDbContext(Action<LearningForAbpDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<LearningForAbpDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<LearningForAbpDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<LearningForAbpDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<LearningForAbpDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LearningForAbpDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<LearningForAbpDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LearningForAbpDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<LearningForAbpDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<LearningForAbpDbContext, Task<T>> func)
        {
            T result;
